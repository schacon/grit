//! `gust commit` — record changes to the repository.
//!
...
use gust_lib::config::ConfigSet;
use gust_lib::error::Error;
use gust_lib::index::Index;
use gust_lib::objects::{serialize_commit, CommitData, ObjectId, ObjectKind};
use gust_lib::repo::Repository;
use gust_lib::state::{resolve_head, HeadState};
use gust_lib::write_tree::write_tree_from_index;
use std::fs;
use std::path::Path;
use time::OffsetDateTime;

/// Arguments for `gust commit`.
...
pub fn run(args: Args) -> Result<()> {
    ...
    if args.amend {
        ...
        if let Some(head_oid) = head.oid() {
            ...
            let commit = gust_lib::objects::parse_commit(&obj.data)?;
        ...
        let merge_heads = gust_lib::state::read_merge_heads(&repo.git_dir)?;
    ...
    if !args.allow_empty && !parents.is_empty() && !args.amend {
        ...
        let parent_commit = gust_lib::objects::parse_commit(&parent_obj.data)?;
...
fn auto_stage_tracked(repo: &Repository, work_tree: &Path) -> Result<()> {
    ...
    for (raw_path, path_str) in &tracked {
        ...
        if abs_path.exists() {
            ...
            let mode = gust_lib::index::normalize_mode(meta.mode());
            let entry = gust_lib::index::entry_from_stat(&abs_path, raw_path, oid, mode)?;
...
fn build_message(args: &Args, repo: &Repository) -> Result<String> {
    ...
    // Check for MERGE_MSG
    if let Some(msg) = gust_lib::state::read_merge_msg(&repo.git_dir)? {
        return Ok(ensure_trailing_newline(&msg));
    }

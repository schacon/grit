//! `gust config` — read and modify Git configuration files.
//!
...
use gust_lib::config::{parse_bool, parse_i64, parse_path, ConfigFile, ConfigScope, ConfigSet};
use std::path::{Path, PathBuf};

/// Arguments for `gust config`.
...
/// Modern subcommand interface for `gust config`.
...
/// Arguments for `gust config unset`.
#[derive(Debug, ClapArgs)]
...
/// Arguments for `gust config list`.
#[derive(Debug, ClapArgs)]
pub struct ListArgs {}

/// Arguments for `gust config rename-section`.
#[derive(Debug, ClapArgs)]
...
/// Arguments for `gust config remove-section`.
#[derive(Debug, ClapArgs)]
...
/// Arguments for `gust config edit`.
#[derive(Debug, ClapArgs)]
pub struct EditArgs {}
...
pub fn run(args: Args) -> Result<()> {
    ...
    match args.positional.len() {
        0 => {
            ...
            bail!("usage: gust config [<options>]");
        }
        1 => {

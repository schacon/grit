//! `gust symbolic-ref` — read, update, and delete symbolic refs.

use anyhow::{bail, Context, Result};
use clap::Args as ClapArgs;
use gust_lib::objects::ObjectId;
use gust_lib::refs::{append_reflog, read_ref_file, Ref};
...
pub fn run(args: Args) -> Result<()> {
    ...
    if args.delete {
        let Some(name) = args.name.as_deref() else {
            bail!("usage: gust symbolic-ref --delete [-q] <name>");
        };
    ...
    match (args.name.as_deref(), args.reference.as_deref()) {
        ...
        _ => bail!("usage: gust symbolic-ref [-m <reason>] <name> <ref>"),
    }
}
...
fn read_symbolic_ref_target(git_dir: &Path, name: &str, recurse: bool) -> Result<Option<String>> {
    ...
    match read_ref_file(&path) {
        ...
        Ok(Ref::Symbolic(mut target)) => {
            ...
            for _ in 0..10 {
                ...
                match read_ref_file(&next_path) {
                    ...
                    Err(gust_lib::error::Error::Io(err))
        ...
        Err(gust_lib::error::Error::Io(err)) if err.kind() == io::ErrorKind::NotFound => {
...
fn is_symbolic_ref(git_dir: &Path, name: &str) -> Result<bool> {
    ...
    match read_ref_file(&path) {
        ...
        Err(gust_lib::error::Error::Io(err)) if err.kind() == io::ErrorKind::NotFound => Ok(false),
        Err(err) => Err(err.into()),
    }
}
...
fn resolve_for_reflog(repo: &Repository, name: &str) -> ObjectId {
    match gust_lib::refs::resolve_ref(&repo.git_dir, name) {
        Ok(oid) => oid,
        Err(_) => zero_oid(),
    }
}

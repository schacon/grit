//! `gust show-ref` — list and verify refs in a local repository.

use anyhow::{bail, Context, Result};
use clap::Args as ClapArgs;
use gust_lib::objects::{ObjectId, ObjectKind};
...
fn run_verify_mode(
    ...
    for reference in &args.refs_or_patterns {
        ...
        let oid = match gust_lib::refs::resolve_ref(&repo.git_dir, reference) {
            Ok(oid) => oid,
...
fn run_pattern_mode(
    ...
    if args.head {
        if let Ok(head_oid) = gust_lib::refs::resolve_ref(&repo.git_dir, "HEAD") {
...
fn collect_loose_refs(
    ...
    for entry in read_dir {
        ...
        if file_type.is_dir() {
            ...
            if let Ok(oid) = gust_lib::refs::resolve_ref(git_dir, &next_relative) {

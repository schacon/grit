//! `gust merge-base` - find best common ancestors for merges.

use anyhow::{bail, Context, Result};
use clap::Args as ClapArgs;
use gust_lib::merge_base::{
...
fn run_default(repo: &Repository, show_all: bool, revisions: Vec<String>) -> Result<()> {
    if revisions.len() < 2 {
        bail!("usage: gust merge-base [-a | --all] <commit> <commit>...");
    }
...
fn run_octopus(repo: &Repository, show_all: bool, revisions: Vec<String>) -> Result<()> {
    if revisions.is_empty() {
        bail!("usage: gust merge-base [-a | --all] --octopus <commit>...");
    }
    let commits = resolve_commit_specs(repo, &revisions)?;
...
fn run_independent(repo: &Repository, show_all: bool, revisions: Vec<String>) -> Result<()> {
    ...
    if revisions.is_empty() {
        bail!("usage: gust merge-base --independent <commit>...");
    }
    let commits = resolve_commit_specs(repo, &revisions)?;
...
fn print_result(mut oids: Vec<gust_lib::objects::ObjectId>, show_all: bool) {
    if oids.is_empty() {
        std::process::exit(1);
    }
    if show_all {

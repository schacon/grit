//! `gust status` — show the working tree status.
//!
//! Displays staged changes, unstaged changes, and untracked files.

use anyhow::{Context, Result};
use clap::Args as ClapArgs;
use gust_lib::diff::{diff_index_to_tree, diff_index_to_worktree, DiffStatus};
use gust_lib::error::Error;
use gust_lib::index::Index;
use gust_lib::objects::parse_commit;
use gust_lib::repo::Repository;
use gust_lib::state::{detect_in_progress, resolve_head, HeadState};
use std::collections::BTreeSet;
use std::fs;
use std::io::{self, Write};
use std::path::Path;

/// Arguments for `gust status`.
...
fn format_short(
    ...
    args: &Args,
    head: &HeadState,
    staged: &[gust_lib::diff::DiffEntry],
    unstaged: &[gust_lib::diff::DiffEntry],
    untracked: &[String],
) -> Result<()> {
...
fn format_long(
    out: &mut impl Write,
    head: &HeadState,
    in_progress: &[gust_lib::state::InProgressOperation],
    staged: &[gust_lib::diff::DiffEntry],
    unstaged: &[gust_lib::diff::DiffEntry],
    untracked: &[String],
) -> Result<()> {

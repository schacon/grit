...
pub fn run(args: Args) -> Result<()> {
    ...
    for op in &ops {
        ...
        if let Some(old_entry) = index.get(op.src.as_bytes(), 0).cloned() {
            ...
            // Mark stat data as "smudged" so subsequent diff/status operations
            // verify content by OID instead of trusting potentially racy mtimes.
            // This avoids missing immediate post-rename edits with unchanged size.
            new_entry.ctime_sec = 0;
            new_entry.ctime_nsec = 0;
            new_entry.mtime_sec = 0;
            new_entry.mtime_nsec = 0;
            new_entry.dev = 0;
            new_entry.ino = 0;
            new_entry.uid = 0;
            new_entry.gid = 0;

...
fn run_geometric(
    ...
    if args.local && grit_lib::pack::read_alternates_recursive(&objects_dir).map_or(false, |v| !v.is_empty()) {
    ...
    let should_run_pack_objects = prom_split > 0
        || split > 0
        || !normal.is_empty()
        || has_loose;
    ...
    if args.write_midx {
        let pref_idx = preferred_pack_index(&pack_dir, pref_stem.as_deref())?;
        let bitmap =
            args.write_bitmap && !args.no_write_bitmap_index && !(args.local && grit_lib::pack::read_alternates_recursive(&objects_dir).map_or(false, |v| !v.is_empty()));
        write_multi_pack_index_with_options(
            &pack_dir,
            &WriteMultiPackIndexOptions {
                preferred_pack_idx: pref_idx,

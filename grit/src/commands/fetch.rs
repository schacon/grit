...
fn fetch_remote(
    ...
    let display_url = resolve_fetch_display_url(git_dir, &url, url_override, &remote_repo)?;
    let effective_tracking_remote = if url_override.is_some() {
        find_remote_for_repository_url(config, git_dir, &remote_repo)
    } else {
    ...
    let cli_refspecs_owned = expand_fetch_cli_tag_args(&args.refspecs)?;
    let cli_refspecs: &[String] = cli_refspecs_owned.as_slice();
    ...
    let (remote_heads, remote_tags) = if use_upload_pack_negotiation {
        ...
        let object_copy_roots = if !cli_refspecs.is_empty() {
    ...
    let remote_symbolic_head_branch = remote_symbolic_head_branch(&remote_repo.git_dir);
    let remote_default_branch_fetch = remote_default_branch_for_fetch_merge(&remote_repo.git_dir);

...
use grit_lib::diff::{
    anchored_unified_diff, count_changes_with_algorithm, count_git_lines, detect_renames,
    diff_index_to_tree, diff_index_to_worktree, diff_tree_to_worktree, diff_trees,
...
fn write_shortstat(
    ...
    for entry in entries {
        let (ins, del) =
            stat_ins_del_for_entry(odb, entry, work_tree, break_rewrites, algo_ctx, algo_cli);
        total_ins += ins;
        total_del += del;
        files_changed += 1;
    }

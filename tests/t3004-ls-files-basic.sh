#!/bin/sh
# Ported from git/t/t3004-ls-files-basic.sh (harness-compatible subset).

test_description='gust ls-files basic'

cd "$(dirname "$0")" || exit 1
. ./test-lib.sh

test_expect_success 'setup repository' '
	gust init repo &&
	cd repo
'

test_expect_success 'ls-files in empty repository' '
	cd repo &&
	: >expect &&
	gust ls-files >actual &&
	test_cmp expect actual
'

test_expect_success 'ls-files with nonexistent path is empty' '
	cd repo &&
	: >expect &&
	gust ls-files doesnotexist >actual &&
	test_cmp expect actual
'

test_expect_success 'ls-files lists tracked paths after update-index --add' '
	cd repo &&
	echo one >one &&
	echo two >two &&
	gust update-index --add one two &&
	cat >expect <<-\EOF &&
	one
	two
	EOF
	gust ls-files >actual &&
	test_cmp expect actual
'

test_expect_success 'ls-files --stage shows mode oid stage path' '
	cd repo &&
	one_oid=$(gust hash-object -w one) &&
	two_oid=$(gust hash-object -w two) &&
	cat >expect <<-EOF &&
	100644 $one_oid 0	one
	100644 $two_oid 0	two
	EOF
	gust ls-files --stage one two >actual &&
	test_cmp expect actual
'

test_done

...
test_expect_failure 'setup bare remote (grit: push to path without remote config)' '
	git init &&
...
test_expect_failure '--all and --branches have the same behavior (grit: push --all not supported)' '
...
test_expect_failure '--all or --branches can not be combined with refspecs (grit: push --all not supported)' '
...
test_expect_failure '--all or --branches can not be combined with --mirror (grit: push --all not supported)' '
...
test_expect_failure '--all or --branches can not be combined with --tags (grit: push --all not supported)' '
...
test_expect_failure '--all or --branches can not be combined with --delete (grit: push --all not supported)' '
...
test_expect_failure '--all or --branches combines with --follow-tags have same behavior (grit: push --all/--follow-tags not supported)' '
	test_when_finished "delete_refs remote-1 \
			   refs/heads/branch-1 \
			   refs/heads/branch-2 \

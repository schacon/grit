...
test_description='gust update-ref basics'
...
test_expect_success 'create refs/heads/main' '
	cd repo &&
	gust update-ref refs/heads/main "$A" &&
	echo "$A" >expect &&
...
test_expect_success 'update refs/heads/main with old-value verification' '
	cd repo &&
	gust update-ref refs/heads/main "$B" "$A" &&
	echo "$B" >expect &&
	cat .git/refs/heads/main >actual &&
	test_cmp expect actual
'
...
test_expect_success 'stale old-value blocks delete' '
	cd repo &&
	test_must_fail gust update-ref -d refs/heads/main "$A" &&
	echo "$B" >expect &&
	cat .git/refs/heads/main >actual &&
	test_cmp expect actual
'
...
test_expect_success 'transaction start/create/commit reports status' '
	...
	commit
	EOF
	gust update-ref --stdin <stdin >actual &&
	cat >expect <<-\EOF &&
	start: ok
	commit: ok
	EOF

...
test_description='gust read-tree -m three-way basics'
...
test_expect_success 'setup base/ours/theirs trees' '
	...
	rm -f .git/index &&
	gust update-index --add shared same-change ours-only theirs-only &&
	tree_o=$(gust write-tree) &&
	echo "$tree_o" >../tree_o &&
	echo ours >shared &&
	echo same-final >same-change &&
	rm -f .git/index &&
	echo ours-base >ours-only &&
	gust update-index --add shared same-change ours-only &&
	tree_a=$(gust write-tree) &&
	echo "$tree_a" >../tree_a &&
	echo theirs >shared &&
	echo same-final >same-change &&
	rm -f .git/index &&
	echo theirs-base >theirs-only &&
	gust update-index --add shared same-change theirs-only &&
	tree_b=$(gust write-tree) &&
	echo "$tree_b" >../tree_b
'
...
test_expect_success 'three-way merge creates staged conflict for divergent path' '
	cd repo &&
	rm -f .git/index &&
	gust read-tree -m "$(cat ../tree_o)" "$(cat ../tree_a)" "$(cat ../tree_b)" &&
	gust ls-files -u >actual &&
...
test_expect_success 'three-way merge resolves identical changes to stage 0' '
	cd repo &&
	gust ls-files --stage same-change >actual &&

...
test_description='gust basic plumbing and index/tree flow'
...
test_expect_success 'write-tree/read-tree round-trip with nested paths and symlink' '
	...
	ln -s path0 path0sym &&
	gust update-index --add path0 path2/file2 path0sym &&
	tree=$(gust write-tree) &&
	gust hash-object path0 >path0_oid &&
	gust hash-object path2/file2 >path2_oid &&
	test "$(cat path0_oid)" = "$PATH0_BLOB_OID" &&
	test "$(cat path2_oid)" = "$PATH2_BLOB_OID" &&
	rm -f .git/index &&
	gust read-tree "$tree" &&
	roundtrip=$(gust write-tree) &&
	test "$roundtrip" = "$tree"
'
...
test_expect_success 'commit-tree records expected tree and parent' '
	cd repo &&
	tree0=$(gust write-tree) &&
	commit0=$(echo base | gust commit-tree "$tree0") &&
	echo "hello path2/extra" >path2/extra &&
	gust update-index --add path2/extra &&
	tree1=$(gust write-tree) &&
	commit1=$(echo child | gust commit-tree "$tree1" -p "$commit0") &&
	gust cat-file -p "$commit1" >commit &&

...
test_description='gust check-ignore subset'
...
test_expect_success 'tracked paths hidden unless --no-index' '
	cd repo &&
	test_must_fail gust check-ignore ignored-but-in-index >actual 2>err &&
	test ! -s actual &&
	gust check-ignore --no-index ignored-but-in-index >actual &&
...
test_expect_success 'nested gitignore negation visible with verbose' '
	cd repo &&
	test_must_fail gust check-ignore a/b/one >actual 2>err &&
	test ! -s actual &&
	gust check-ignore -v a/b/one >actual &&
...
test_expect_success 'directory pattern applies to directory and descendants' '
	cd repo &&
	gust check-ignore a/b/ignored-dir a/b/ignored-dir/file >actual &&
	cat >expect <<-\EOF &&
	a/b/ignored-dir
	a/b/ignored-dir/file
	EOF
	test_cmp expect actual &&
	gust check-ignore -v a/b/ignored-dir/file >actual &&
...
test_expect_success '--stdin default mode' '
	...
	a/b/twooo
	EOF
	gust check-ignore --stdin <stdin >actual &&
	cat >expect <<-\EOF &&
	one
	a/b/twooo
	EOF
...
test_expect_success '--stdin verbose non-matching mode' '
	...
	a/b/twooo
	EOF
	gust check-ignore --stdin -v -n <stdin >actual &&
	cat >expect <<-\EOF &&
	.gitignore:1:one	one
	::	not-ignored
...
test_expect_success '--stdin -z emits NUL-delimited records' '
	...
	gust check-ignore --stdin -z <stdin0 >actual0 &&
	printf "one\0a/b/twooo\0" >expect0 &&
	test_cmp expect0 actual0 &&
	gust check-ignore --stdin -z -v <stdin0 >actual0 &&
...
test_expect_success 'info/exclude and core.excludesfile precedence' '
	cd repo &&
	gust check-ignore -v per-repo a/per-repo >actual &&
	cat >expect <<-\EOF &&
	...
	EOF
	gust check-ignore -v globalone per-repo globalthree a/globalthree globaltwo >actual &&
	cat >expect <<-\EOF &&

...
///
/// Returns the object id at the leaf (blob, tree, gitlink, or symlink target blob), matching
/// `git rev-parse <rev>:<path>` for non-blob tree entries.
fn resolve_tree_path(repo: &Repository, tree_oid: &ObjectId, path: &str) -> Result<ObjectId> {
    let (oid, _) = walk_tree_to_treeish_entry(repo, tree_oid, path)?;
    Ok(oid)
}
...
/// Walk from `tree_oid` to the leaf named by `path` and return OID + raw tree mode.
fn walk_tree_to_treeish_entry(
    repo: &Repository,
    tree_oid: &ObjectId,
    path: &str,
) -> Result<(ObjectId, u32)> {
    ...
    for entry in entries {
        ...
        if name == first {
            ...
            return walk_tree_to_treeish_entry(repo, &entry.oid, &rest.join("/"));
        }
    }
...
fn walk_tree_to_blob_entry(
    ...
    let (oid, mode) = walk_tree_to_treeish_entry(repo, tree_oid, path)?;
    if mode == crate::index::MODE_TREE {
        return Err(Error::InvalidRef(format!("'{path}' is a tree, not a blob")));
    }
    if mode == crate::index::MODE_GITLINK {
        return Err(Error::InvalidRef(format!(

...
///
/// Git accepts `rev:path` when the leaf is a **blob or tree** (e.g. `HEAD:subdir` for a
/// subdirectory tree). Only [`walk_tree_to_blob_entry`] is blob-only — use a dedicated walk here.

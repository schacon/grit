...
fn parse_midx_header(data: &[u8]) -> Result<(MidxFileHeader, usize)> {
    ...
    if version != MIDX_VERSION_V1 {
        return Err(Error::CorruptObject(format!("unsupported MIDX version {version}")));
    }
    let hash_len = data[5];
    if hash_len != 1 {
        return Err(Error::CorruptObject("unsupported MIDX hash version".to_owned()));
    }
...
fn find_chunk(data: &[u8], header_end: usize, chunk_id: u32) -> Result<(usize, usize)> {
    ...
    if data.len() < toc_end + 20 {
        return Err(Error::CorruptObject("truncated MIDX chunk table".to_owned()));
    }
    for i in 0..n {
...
pub fn midx_checksum_hex(objects_dir: &Path) -> Result<String> {
    ...
    let data = fs::read(&path).map_err(Error::Io)?;
    if data.len() < 20 {
        return Err(Error::CorruptObject("midx too small for checksum".to_owned()));
    }
...
pub fn read_midx_preferred_idx_name(objects_dir: &Path) -> Result<String> {
    ...
    if entry_base + 8 > data.len() || entry_base + 8 > ooff_off + ooff_len {
        return Err(Error::CorruptObject("bad MIDX object-offsets index".to_owned()));
    }
    let pack_id = u32::from_be_bytes(data[entry_base..entry_base + 4].try_into().unwrap());
    let idx = usize::try_from(pack_id).map_err(|_| {
        Error::CorruptObject("pack id overflow in multi-pack-index".to_owned())
    })?;
    names
        .get(idx)
        .cloned()

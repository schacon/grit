...
- [x] `t4252-am-options` ████████████████████ 8/8 (0 left) — git am with options and not losing them (`am` now accepts and persists option set across interrupted sessions for `--whitespace=fix`, `-C<n>`, `-p<n>`, `--directory=<dir>`, and `--reject`; patch application now delegates to shared `apply` machinery with in-memory patch input to preserve git-compatible path transforms/context matching/reject behavior; reject-mode sessions now write `.git/rebase-apply/apply-opt` including `--reject`, and `apply` exposes a reusable `run_with_patch_input` entry for internal callers)
...
- [x] `t4017-diff-retval` ████████████████████ 38/38 (0 left) — Return value of diffs (`diff-tree -S` now filters `--exit-code` on pickaxe results; `diff --check` now detects conflict markers (including `conflict-marker-size` attributes), returns git-compatible 1/2/3 exit codes with `--exit-code`, accepts global `--no-pager`, and reports unknown trailing flags as usage errors with exit 129)
- [x] `t4035-diff-quiet` ████████████████████ 23/23 (0 left) — Return value of diffs (`diff-tree --quiet --stdin` now preserves stdin echo line in quiet mode while still suppressing diff body; `diff --quiet` outside-repo and mixed inside/outside paths now auto-enable no-index semantics and honor whitespace-ignore modes for no-index comparisons)
- [x] `t4213-log-tabexpand` ████████████████████ 9/9 (0 left) — log/show --expand-tabs (`show` now accepts `--expand-tabs[=<n>]` and `--no-expand-tabs`, applies Git-compatible default tab expansion by pretty format (including `--pretty` alias as medium), and preserves expected short-format title indentation while expanding body lines according to selected tab width)
- [x] `t4058-diff-duplicates` ███████████████████░ 15/16 (1 left) — test tree diff when trees have duplicate entries (`diff-tree` now accepts `--no-abbrev`, preserves duplicate path entries in tree diffs, and avoids self-renames under `-M`; duplicate-tree corruption now surfaces as `error: corrupted cache-tree has entries not present in index` across `read-tree`, `reset --hard`, and `diff HEAD`; status/reset duplicate-path handling now matches non-TODO assertions, with only upstream test 14 still flagged as TODO known breakage)
...
- [x] `t4067-diff-partial-clone` ████████████████████ 9/9 (0 left) — behavior of diff when reading objects in a partial clone (`diff` now correctly reclassifies bare-repo revision args without `--`, batches blob prefetch/trace output (including rename and break-rewrite paths), skips gitlink OIDs, `checkout` lazily hydrates missing blobs in partial clones, and gitlink staging now resolves submodule gitdirs via `.git` gitfile paths; local mirror `./scripts/run-tests.sh` remains 0/9 due its simplified `tests/test-lib.sh` helper using `git config -C` subcommand argument order that diverges from upstream harness invocation)
- [x] `t4208-log-magic-pathspec` ████████████████████ 21/21 (0 left) — magic pathspec tests using git-log (`log` now disambiguates revision-vs-pathspec magic tokens with Git-compatible ambiguity/errors, supports `--merge` option parsing, `rev-parse` rejects bare `:/` as a revision while preserving `:/<msg>` search, and shared pathspec matching treats `:/` as top-of-repo pathspec so `log HEAD -- :/` from subdirs behaves like upstream)
- [x] `t4212-log-corrupt` ████████████████████ 13/13 (0 left) — git log with invalid commit headers (`fsck` now validates commit ident lines and reports malformed identities as `invalid.author`/`invalid.committer`; `log` date parsing now treats unparsable and whitespace-only timestamps as sentinel values with Git-compatible header/placeholder rendering (`Date:` shows epoch for headers while `%ad` becomes empty for malformed non-overflow dates, `%at/%ct` become empty for malformed dates, and overflow numerics map to epoch); `rev-list` now supports `--since/--until` date filtering and treats malformed commit dates as sentinel timestamps so corrupt-header traversals match upstream behavior)
- [x] `t4128-apply-root` ████████████████████ 12/12 (0 left) — apply same filename (`apply --directory` now normalizes prefixes (`./`, duplicate slashes, internal `..`) with explicit rejection for leading `../`; quoted diff paths in both `diff --git` and `---/+++` headers are now decoded from Git C-style escapes before `-p`/`--directory` transforms, fixing quoted-filename index updates)

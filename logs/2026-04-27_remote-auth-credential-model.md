...
- Implemented `http.emptyAuth` as a no-prompt empty Basic auth attempt in the current HTTP stack; `http.proactiveAuth` remains ignored when `emptyAuth` is enabled.
...
- Documented the remaining HTTP failures as non-auth transport gaps: `t5551` is down to redirect/SHA-256 empty clone behavior after auth/redaction coverage, `t5539` is shallow/deepen state, and HTTPS-over-HTTP-proxy still needs CONNECT/TLS support.

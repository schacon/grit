...
- Implemented `gust symbolic-ref` with read/update/delete flows, `--short`, `--no-recurse`, `-q`, `-d`, and target validation rules.
- Implemented `gust show-ref` with pattern listing, `--branches`/`--tags`, `--verify`, `--exists`, `-d/--dereference`, `-s/--hash`, and quiet/exit-code behavior used by ported tests.
